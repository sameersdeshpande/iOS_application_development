...
class ClaimsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UIPickerViewDataSource, UIPickerViewDelegate, UISearchBarDelegate {
    ...
    @IBOutlet weak var claimscroll: UIScrollView!
    ...
    deinit {
        ...
        // Adjust the scroll view content inset when the keyboard appears
        @objc func keyboardWillShow(notification: Notification) {
            if let userInfo = notification.userInfo,
               let keyboardFrame = userInfo[UIResponder.keyboardFrameEndUserInfoKey] as? CGRect {
                let keyboardHeight = keyboardFrame.height
                UIView.animate(withDuration: 0.3) {
                    self.claimscroll.contentInset.bottom = keyboardHeight
                    self.claimscroll.scrollIndicatorInsets.bottom = keyboardHeight
                }
            }
        }

        // Reset the scroll view content inset when the keyboard disappears
        @objc func keyboardWillHide(notification: Notification) {
            UIView.animate(withDuration: 0.3) {
                self.claimscroll.contentInset.bottom = 0
                self.claimscroll.scrollIndicatorInsets.bottom = 0
            }
        }
//    // This method is called when a UITextField becomes the first responder (when user starts typing)
//     func textFieldDidBeginEditing(_ textField: UITextField) {
//         // Ensure the active text field is visible within the scroll view when editing begins
//         let rect = textField.frame
//         claimscroll.scrollRectToVisible(rect, animated: true)
//     }
    ...
    // Ensure the scroll view has the correct content size
     func scrollViewDidScroll(_ scrollView: UIScrollView) {
         // You can debug here to ensure contentSize and scroll offsets are correct
